...
if(isset($_SESSION['empId']) AND $_SESSION['positionAccess'] <= 2){
  ...
  <body>
    ...
    if(isset($_POST['submit'])){
      $name=$_POST['name'];
      $there=mysqli_query($con, "SELECT MAX(suppId) AS 'there' FROM supplier");
      $hello=mysqli_fetch_array($there);
      $suppId="";
      if(!$hello['there']){
        $suppId="000001";
      }else{
        $suppId=sprintf("%06d", ($hello['there']+1));
      }
      $suppPhone=$_POST['phone'];
      $suppMail=$_POST['mail'];
      $suppAddress=$_POST['address'];
      $query="INSERT INTO supplier VALUES('$suppId', '$name', '$suppAddress', '$suppPhone', '$suppMail', 1)";
      queryRun($query);
    } ?>
    ...
    <div class="container">
      ...
      <form class="form-horizontal" action="" method=post>
        ...
        <div class="form-group">
          ...
          <div class="col-sm-4">
            <textarea name=address id=address rows=3 cols=50></textarea>
          </div>
        </div>
        ...
        <div class="form-group">
          <div class="col-sm-offset-3 col-sm-9">
            ...
            <a href="mansupp.php"><input type="button" class="btn btn-default" value=Cancel></a>
          </div>
        </div>
